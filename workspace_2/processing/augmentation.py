...
class TimeDownsamplingTransformer(Transformer[np.ndarray, np.ndarray]):
    def __init__(self, downsampling_factor: float = 1.0, sample_rate: int = None):
        ...
        super().__init__(
            f'spectrogram_time_downsampling_{downsampling_factor}')
    ...
    def transform(self, input: np.ndarray) -> np.ndarray:
        ...
        if len(input.shape) == 2:
            if USE_PYRUBBERBAND:
                ...
                times_downsampled = max(
                    1, int(times / self.downsampling_factor))
            ...
            raise ValueError(
                f'Expected input data with a dimensionality of 2, got {len(input.shape)} (exact shape was {repr(input.shape)}).')
...
class DiverseLoudnessTransformer(Transformer[np.ndarray, np.ndarray]):
    ...
    def transform(self, x: np.ndarray) -> np.ndarray:
        ...
        weigthing = np.concatenate((weigthing[0:(
            len(x)-len(weigthing))//2], weigthing, weigthing[-(len(x)-len(weigthing))//2:]))
