...
DATASET_FOLDER = "C:/Users/User/Documents/GitHub/uni/ddspzart/workspace_2/source_separation/data/OneExampleRecording"
...
class Track():
    ...
    def init_targets(self):
        stem_ids = [int(stem[-7:-5])
                    for stem in os.listdir(f"{self.track_folder}/stems")]
        # tic = time.perf_counter()
        if os.path.exists(self.save_targets_file) and self.split == "train":
            ...
            self.targets = [Target(self.track_id, stem_id, split=self.split,
                                   seq_duration=self.seq_duration) for stem_id in stem_ids]
    ...
    def __repr__(self) -> str:
        # with {[t.instrument for t in self.targets]}."
    ...
    def get_all_audio_seqs(self):
        ...
        stem_audio = np.zeros(
            (int(self.seq_duration*self.sample_rate), len(self.targets)))
        for i, target in enumerate(self.targets):
    ...
    def get_selected_audio_seqs(self, target: Target):
        ...
        if len(stem_audio) == 0 or (len(stem_audio) != self.seq_duration * self.sample_rate):
...
class SlakhDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.all_tracks = [Track(int(id_str[-5:]), split=split,
                                 seq_duration=seq_duration) for id_str in track_folders]
        self.filter_target()
            ...
            for ta, target in enumerate(track.targets):
                if target.instrument == self.target:
                    if cnt >= 0:  # not used currently
                        ...
                        # I just add all elements...
                        logging.info("Do not add.")
                    cnt += 1
