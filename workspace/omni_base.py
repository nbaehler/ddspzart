...
class BaseTranscription(metaclass=ABCMeta):
    ...
    def _resolve_model_path(self, model_path=None):
        ...
        if model_path is None:
            ...
            if not cand_dirs:  # pylint: disable=R1720
                ...
                logger.warning(
                    "There are multiple checkpoints in the directory. Default to use %s",
                    cand_dirs[0],
                )
    ...
    def _get_model_from_yaml(
        self, arch_path, custom_objects=None
    ):  # pylint: disable=R0201
        return model_from_yaml(
            open(arch_path, "r").read(), custom_objects=custom_objects
        )

    def _resolve_feature_output_path(
        self, dataset_path, settings
    ):  # pylint: disable=R0201
    ...
    def _output_midi(
        self, output, input_audio, midi=None, verbose=True, instrument=None
    ):
        if output is None:
            return None
...
class Label:
    ...
    def __eq__(self, val):
        ...
        if (
            abs(self.start_time - val.start_time) < epsilon
...
class BaseDatasetLoader:
    ...
    def __init__(
        ...
        length_map = {
            hdf: len(hdf_ref[feat_col_name]) for hdf, hdf_ref in self.hdf_refs.items()
        }
