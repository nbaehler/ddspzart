...
def roll_down_sample(data, base=88):
    ...
    Down sample the feature size from 354 to 88 for inferring the notes.
    ...
    assert (
        total_roll % base == 0
    ), f"Wrong length: {total_roll}, {total_roll} % {base} should be zero!"
...
def down_sample(pred):
    ...
    Down sample the feature size from 354 to 88 for inferring the notes from a multi-channel prediction.
...
def infer_pitch(pitch, shortest=10, offset_interval=6):
    ...
    notes = [
        {"start": peaks[i], "end": peaks[i + 1], "stren": pitch[peaks[i], 2]}
...
def infer_piece(piece, shortest_sec=0.05, offset_sec=0.12, t_unit=0.02):
    ...
    assert (
        piece.shape[1] == 88
    ), "Please down sample the pitch to 88 first (current: {}).format(piece.shape[1])"
    ...
    for i in range(88):
        ...
        pns = infer_pitch(
            pitch,
            shortest=round(shortest_sec / t_unit),
            offset_interval=round(offset_sec / t_unit),
        )
        for note in pns:
            note["pitch"] = i
            notes.append(note)
...
def find_min_max_stren(notes):
    """Function for determine the note velocity according to prediction value.
...
def norm_split_onset_dura(
    ...
    upper_pred = norm_onset_dura(
        upper_pred, onset_th, dura_th, interpolate=interpolate, normalize=normalize
    )

    lower_range = range(4 * split_bound)
    lower_pred = pred[:, lower_range]
    lower_pred = norm_onset_dura(
        lower_pred,
        lower_onset_th,
        dura_th,
...
def entropy(data, bins=200):
    ...
    return 0 - sum(prob * math.log(prob, math.e) for prob in probs)
...
def note_inference(
    ...
    if "note" in mode:
        ...
        return to_midi(notes, t_unit=t_unit / 2)
        ...
        return to_midi(notes, t_unit=t_unit)
...
def multi_inst_note_inference(
    ...
    """Function for inferring raw multi-instrument predictions.
    ...
    if mode in ["note-stream", "note", "pop-note-stream", "frame-stream", "frame"]:
        ch_per_inst = 2
    ...
    for i in range(ch_per_inst):
        ...
        item = pred[
            :, :, [it * ch_per_inst + i + 1 for it in range(iters)]
        ]  # noqa: E226
    ...
    for i in range(iters):
        ...
        # Assign instrument class to the inferred MIDI according to its channel index
