...
class SourceSeparation():
    ...
    def __init__(self, mean, scale) -> None:
        self.stft = openunmix.transforms.TorchSTFT() # -> shape (nb_samples, nb_channels, nb_bins, nb_frames, complex=2)
        self.spec = openunmix.transforms.ComplexNorm(mono=True) # -> shape (nb_samples, nb_channels(=1 if mono), nb_bins, nb_frames)
...
class SourceSeperationLight(pl.LightningModule):
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.RMSprop(self.model.unmix.parameters(), lr=0.005)
